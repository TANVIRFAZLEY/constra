<?php
/**
 * Constra - Construction WP Theme functions and definitions
 *
...
function constra_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'constra' ),
		)
	);
...
function constra_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'constra' ),
			'id'            => 'sidebar-1',
...
function constra_scripts() {
	wp_enqueue_style( 'constra-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'constra-style', 'rtl', 'replace' );

	wp_enqueue_script( 'constra-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
